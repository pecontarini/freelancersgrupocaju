...
const tipoIcons: Record<string, any> = {
  ENTRADA: ArrowUpCircle, SAIDA: ArrowDownCircle, AJUSTE: RefreshCw, TRANSFERENCIA: ArrowRightLeft,
};
const tipoColors: Record<string, string> = {
  ENTRADA: "bg-green-600", SAIDA: "bg-red-600", AJUSTE: "bg-yellow-600", TRANSFERENCIA: "bg-blue-600",
};
...
export function Movimentacao() {
  ...
  const handleSubmit = () => {
    ...
    createMov.mutate({
      setor_item_id: setorItemId, loja_id: effectiveUnidadeId, tipo_movimentacao: tipo,
      quantidade: parseFloat(quantidade),
      setor_destino_id: tipo === "TRANSFERENCIA" ? setorDestinoId || undefined : undefined,
      responsavel: profile?.full_name || "—", observacao: observacao || undefined,
    }, { onSuccess: () => { setSetorItemId(""); setQuantidade(""); setObservacao(""); setSetorDestinoId(""); } });
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><Label>Tipo</Label>
              <Select value={tipo} onValueChange={setTipo}><SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="ENTRADA">Entrada</SelectItem><SelectItem value="SAIDA">Saída</SelectItem>
                  <SelectItem value="AJUSTE">Ajuste</SelectItem><SelectItem value="TRANSFERENCIA">Transferência</SelectItem>
                </SelectContent></Select></div>
            <div><Label>Item</Label>
              <Select value={setorItemId} onValueChange={setSetorItemId}><SelectTrigger><SelectValue placeholder="Selecionar item" /></SelectTrigger>
                <SelectContent>{setorItems?.map((si: any) => (
                  <SelectItem key={si.id} value={si.id}>{si.items_catalog?.name} ({si.setores?.nome})</SelectItem>
                ))}</SelectContent></Select></div>
            <div><Label>Quantidade</Label><Input type="number" min="0" step="0.01" value={quantidade} onChange={(e) => setQuantidade(e.target.value)} /></div>
            {tipo === "TRANSFERENCIA" && (
              <div><Label>Setor Destino</Label>
                <Select value={setorDestinoId} onValueChange={setSetorDestinoId}><SelectTrigger><SelectValue placeholder="Destino" /></SelectTrigger>
                  <SelectContent>{setores?.map((s: any) => <SelectItem key={s.id} value={s.id}>{s.nome}</SelectItem>)}</SelectContent></Select></div>
            )}
          </div>
          <div><Label>Observação</Label><Textarea value={observacao} onChange={(e) => setObservacao(e.target.value)} placeholder="Opcional" rows={2} /></div>
          <Button onClick={handleSubmit} disabled={!setorItemId || !quantidade || createMov.isPending}><Plus className="h-4 w-4 mr-2" />Registrar</Button>
        </CardContent>
      </Card>
          ...
          {isLoading ? (
                ...
                <Card key={m.id} className="border">
                  <CardContent className="p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">{format(new Date(m.data_movimentacao), "dd/MM/yy HH:mm")}</span>
                    </div>
                    <p className="font-medium text-sm">{m.setor_items?.items_catalog?.name}</p>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">{m.setor_items?.setores?.nome}</span>
                      <span className="font-mono font-bold">Qtd: {m.quantidade}</span>
                    </div>
            ...
            <Table>
              <TableHeader><TableRow>
                ...
                <TableHead>Setor</TableHead><TableHead className="text-right">Qtd</TableHead>
                <TableHead>Responsável</TableHead><TableHead>Obs</TableHead>
                ...
                {movimentacoes.map((m: any) => {
                  ...
                  return (
                    <TableRow key={m.id}>
                      ...
                      <TableCell><Badge className={`${tipoColors[m.tipo_movimentacao] || ""} text-xs`}><Icon className="h-3 w-3 mr-1" />{m.tipo_movimentacao}</Badge></TableCell>
