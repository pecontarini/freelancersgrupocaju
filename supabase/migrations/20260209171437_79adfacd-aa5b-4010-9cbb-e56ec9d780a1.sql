...
BEGIN
  ...
  purchases AS (
    SELECT m.cmv_item_id, COALESCE(SUM(m.quantidade), 0::numeric) AS total_qty
  ...
  sales AS (
    SELECT
      sm.cmv_item_id,
      COALESCE(SUM(ds.quantity * sm.multiplicador), 0::numeric) AS total_consumption
    FROM daily_sales ds
  ...
  SELECT
    ...
    COALESCE(p.total_qty, 0::numeric) AS purchases_qty,
    COALESCE(s.total_consumption, 0::numeric) AS sales_consumption,
    (COALESCE(ic.quantidade, 0::numeric) + COALESCE(p.total_qty, 0::numeric) - COALESCE(s.total_consumption, 0::numeric)) AS theoretical_final,
    COALESCE(fc.quantidade, 0::numeric) AS real_final_stock,
    COALESCE(fc.preco_custo_snapshot, ai.preco_custo_atual) AS final_cost,
    (COALESCE(fc.quantidade, 0::numeric) - (COALESCE(ic.quantidade, 0::numeric) + COALESCE(p.total_qty, 0::numeric) - COALESCE(s.total_consumption, 0::numeric))) AS divergence,
    ((COALESCE(ic.quantidade, 0::numeric) + COALESCE(p.total_qty, 0::numeric) - COALESCE(s.total_consumption, 0::numeric)) - COALESCE(fc.quantidade, 0::numeric))
