...
export function CatalogoItens() {
  ...
  const openLinkDialog = (item: any) => {
    setLinkItem(item); setLinkSetorId(""); setLinkMin("0"); setLinkMax(""); setLinkPonto(""); setEditMode(false); setLinkOpen(true);
  };
  const openEditDialog = (item: any, setorItem: any) => {
    setLinkItem(item); setLinkSetorId(setorItem.setor_id);
    setLinkMin(String(setorItem.estoque_minimo)); setLinkMax(setorItem.estoque_maximo != null ? String(setorItem.estoque_maximo) : "");
    setLinkPonto(setorItem.ponto_pedido != null ? String(setorItem.ponto_pedido) : "");
    setEditMode(true); setEditSetorItemId(setorItem.id); setLinkOpen(true);
  };
  const handleSaveLink = () => {
    if (!effectiveUnidadeId) return;
    if (editMode) {
      updateLink.mutate({ id: editSetorItemId, estoque_minimo: parseInt(linkMin) || 0, estoque_maximo: linkMax ? parseInt(linkMax) : undefined, ponto_pedido: linkPonto ? parseInt(linkPonto) : undefined },
        { onSuccess: () => setLinkOpen(false) });
    } else {
      if (!linkSetorId || !linkItem) return;
      createLink.mutate({ catalog_item_id: linkItem.id, setor_id: linkSetorId, loja_id: effectiveUnidadeId, estoque_minimo: parseInt(linkMin) || 0, estoque_maximo: linkMax ? parseInt(linkMax) : undefined, ponto_pedido: linkPonto ? parseInt(linkPonto) : undefined },
        { onSuccess: () => setLinkOpen(false) });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className={isMobile ? "space-y-2" : "flex flex-wrap gap-3"}>
        ...
        <div className={isMobile ? "grid grid-cols-2 gap-2" : "flex gap-3"}>
          <Select value={filterGrandeGrupo} onValueChange={setFilterGrandeGrupo}>
            <SelectTrigger><SelectValue placeholder="Grande Grupo" /></SelectTrigger>
            <SelectContent><SelectItem value="all">Todos</SelectItem>{grandeGrupos.map((g) => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={filterGrupo} onValueChange={setFilterGrupo}>
            <SelectTrigger><SelectValue placeholder="Grupo" /></SelectTrigger>
            <SelectContent><SelectItem value="all">Todos</SelectItem>{grupos.map((g) => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent>
          </Select>
          ...
          <Select value={filterUtensilio} onValueChange={setFilterUtensilio}>
            <SelectTrigger><SelectValue placeholder="Utensílio" /></SelectTrigger>
        ...
        <div className={isMobile ? "flex items-center justify-between" : "flex items-center gap-3"}>
          <Select value={String(pageSize)} onValueChange={(v) => { setPageSize(Number(v)); setCurrentPage(1); }}>
            ...
            <SelectContent>
              <SelectItem value="25">25/página</SelectItem><SelectItem value="50">50/página</SelectItem>
              <SelectItem value="100">100/página</SelectItem><SelectItem value="250">250/página</SelectItem>
            </SelectContent>
          </Select>
          <span className="text-xs text-muted-foreground">{filtered.length} itens</span>
          {!isMobile && <Button variant="outline" disabled title="Importação CSV (em breve)"><Upload className="h-4 w-4 mr-2" />CSV</Button>}
        </div>
      </div>
      ...
      <Card>
        <CardContent className={isMobile ? "p-0" : "p-0"}>
          {isLoading ? (
                  ...
                  <div key={item.id} className="p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        {item.is_utensilio && <Badge variant="outline" className="text-[10px]">UT</Badge>}
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openLinkDialog(item)}>
                          <Link2 className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap text-xs">
                      {item.grande_grupo && <span className="text-muted-foreground">{item.grande_grupo}</span>}
                      {item.grupo && <span>· {item.grupo}</span>}
                      <span className="ml-auto font-mono cursor-pointer hover:text-primary"
                    ...
                    {costEditId === item.id && (
                      <div className="flex items-center gap-2">
                        <Input type="number" step="0.01" min="0" className="h-8 flex-1" value={costEditValue}
                          onChange={(e) => setCostEditValue(e.target.value)} autoFocus
                          onKeyDown={(e) => {
                            if (e.key === "Enter") { updateCost.mutate({ id: item.id, preco_custo: parseFloat(costEditValue) || 0 }); setCostEditId(null); }
                            else if (e.key === "Escape") setCostEditId(null);
                          }} />
                        <Button size="sm" className="h-8" onClick={() => { updateCost.mutate({ id: item.id, preco_custo: parseFloat(costEditValue) || 0 }); setCostEditId(null); }}>OK</Button>
                      </div>
                    )}
                    {links.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {links.map((l: any) => (
                          <Badge key={l.id} variant="secondary" className="text-[10px] cursor-pointer" onClick={() => openEditDialog(item, l)}>
                            {setores?.find((s: any) => s.id === l.setor_id)?.nome || "?"} <Edit2 className="h-2.5 w-2.5 ml-0.5" />
                          </Badge>
                        ))}
                      </div>
                    )}
            ...
            <Table>
              <TableHeader><TableRow>
                <TableHead>Código</TableHead><TableHead>Item</TableHead><TableHead>UM</TableHead>
                <TableHead className="text-right">Custo Unit.</TableHead><TableHead>Grande Grupo</TableHead>
                <TableHead>Grupo</TableHead><TableHead>Setores Vinculados</TableHead><TableHead></TableHead>
                ...
                {paginatedItems.map((item: any) => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right">
                        {costEditId === item.id ? (
                          <div className="flex items-center gap-1 justify-end">
                            <Input type="number" step="0.01" min="0" className="w-24 text-right h-8" value={costEditValue}
                              onChange={(e) => setCostEditValue(e.target.value)} autoFocus
                              onKeyDown={(e) => {
                                if (e.key === "Enter") { updateCost.mutate({ id: item.id, preco_custo: parseFloat(costEditValue) || 0 }); setCostEditId(null); }
                                else if (e.key === "Escape") setCostEditId(null);
                          ...
                          <span className="cursor-pointer hover:text-primary font-mono text-xs"
                            onClick={() => { setCostEditId(item.id); setCostEditValue(String(item.preco_custo || 0)); }}>
                            R$ {(item.preco_custo || 0).toFixed(2)}
                          </span>
                        )}
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {links.length === 0 ? <span className="text-xs text-muted-foreground">—</span> : links.map((l: any) => (
                      ...
                      <TableCell><Button variant="ghost" size="sm" onClick={() => openLinkDialog(item)}><Link2 className="h-4 w-4" /></Button></TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
          ...
          {totalPages > 1 && (
            <div className={`flex items-center ${isMobile ? "flex-col gap-2 py-3" : "justify-between px-4 py-3"} border-t`}>
              ...
              <Pagination>
                <PaginationContent>
                  ...
                  {!isMobile && pageNumbers.map((pg, i) =>
                    pg === "ellipsis" ? <PaginationItem key={`e${i}`}><PaginationEllipsis /></PaginationItem> :
                    <PaginationItem key={pg}><PaginationLink isActive={pg === currentPage} onClick={() => setCurrentPage(pg as number)} className="cursor-pointer">{pg}</PaginationLink></PaginationItem>
                  )}
                  {isMobile && <span className="text-xs px-2">{currentPage}/{totalPages}</span>}
      ...
      <Dialog open={linkOpen} onOpenChange={setLinkOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editMode ? "Editar Vínculo" : "Vincular a Setor"}</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            <p className="text-sm font-medium">{linkItem?.name} <span className="text-muted-foreground">({linkItem?.code})</span></p>
            {!editMode && (
              <div><Label>Setor</Label>
                <Select value={linkSetorId} onValueChange={setLinkSetorId}><SelectTrigger><SelectValue placeholder="Selecionar setor" /></SelectTrigger>
