...
export function VisaoConsolidada() {
  ...
  const itemsWithStatus = useMemo(() => {
    ...
    return setorItems.map((si: any) => {
      ...
      const status = si.ponto_pedido && saldo < si.ponto_pedido ? "critico" : saldo < si.estoque_minimo ? "alerta" : "ok";
  ...
  const grupos = useMemo(() => {
    ...
    itemsWithStatus.forEach((i: any) => { if (i.items_catalog?.grupo) g.add(i.items_catalog.grupo); });
  ...
  if (!effectiveUnidadeId) {
    return <Card><CardContent className="py-10 text-center text-muted-foreground">Selecione uma unidade para visualizar o estoque.</CardContent></Card>;
  }

  const statusBadge = (status: string) => (
    <Badge variant={status === "ok" ? "default" : status === "alerta" ? "outline" : "destructive"}
      className={status === "ok" ? "bg-green-600" : status === "alerta" ? "text-yellow-600 border-yellow-400" : ""}>
  ...
  return (
    <div className="space-y-6">
          ...
          <Card key={s.id} className="cursor-pointer hover:ring-2 ring-primary/30 transition-all" onClick={() => setFilterSetor(filterSetor === s.id ? "all" : s.id)}>
            <CardContent className="p-4 text-center">
              ...
              <div className="flex justify-center gap-2 mt-1">
                {s.alerta > 0 && <Badge variant="outline" className="text-yellow-600 border-yellow-400 text-xs"><AlertTriangle className="h-3 w-3 mr-1" />{s.alerta}</Badge>}
                {s.critico > 0 && <Badge variant="destructive" className="text-xs"><AlertCircle className="h-3 w-3 mr-1" />{s.critico}</Badge>}
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className={isMobile ? "space-y-2" : "flex flex-wrap gap-3"}>
        ...
        <div className={isMobile ? "grid grid-cols-3 gap-2" : "flex gap-3"}>
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <Card><CardContent className="py-8 text-center text-muted-foreground">
              {setorItems?.length === 0 ? "Nenhum item vinculado." : "Nenhum item encontrado."}
            </CardContent></Card>
              ...
              <CardContent className="p-4 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{item.items_catalog?.code}</p>
                  </div>
                  {statusBadge(item.status)}
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge variant="secondary" className="text-xs">{item.setores?.nome}</Badge>
                  {item.items_catalog?.grupo && <span className="text-xs text-muted-foreground">{item.items_catalog.grupo}</span>}
                </div>
                <div className="grid grid-cols-4 gap-2 text-center bg-muted/30 rounded-md p-2">
                  <div>
                    <p className="text-[10px] text-muted-foreground">Saldo</p>
                    <p className="font-bold font-mono text-sm">{item.saldo}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground">Mín</p>
                    <p className="font-mono text-sm">{item.estoque_minimo}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground">Custo</p>
                    <p className="font-mono text-xs">R${item.custo.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground">Total</p>
                    <p className="font-mono text-xs font-semibold">R${item.valorTotal.toFixed(2)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
